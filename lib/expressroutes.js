...
function valueAccessor(param, consumes) {
    ...
    if (param.in === 'formData') {
        return {
            get: function(req, key) {
                return param.type === 'file' ? (req.files && req.files[key] ? req.files[key][0].originalname : undefined) : req.body[key];
            },
            set: function(req, key, val) {
 ...
 * @param routeSpec
 */
function makeExpressRoute(router, mountpath, route, securityDefinitions, multerInst) {
    ...
    if (route.consumes && route.consumes.indexOf('multipart/form-data') !== -1) {
        var fileFields = [];
        for (var i in route.validators) {
            var validator = route.validators[i];
            if (validator.parameter.type === 'file' && validator.parameter.in === 'formData') {
                fileFields.push({ name: validator.parameter.name, maxCount: 1 });
            }
        }

        before.push(function (req, res, next) {
            multerInst.fields(fileFields)(req, res, function (error) {
                if (error) {
                    error.status = 400;
                    error.name = 'ValidationError';

                    if (error.code === 'LIMIT_UNEXPECTED_FILE') {
                        error.message = 'Unexpected field: \"' + error.field + '\"';
                    }
...
function expressroutes(router, options) {
    var routes, mountpath, multerInst, routePath, routesMethod = {};
    ...
    multerInst = multer(options.multer || {});
    ...
    routes.forEach(function (route) {
        makeExpressRoute(router, mountpath, route, options.api.securityDefinitions, multerInst);
