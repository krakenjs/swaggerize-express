...
function makeValidator(validator, consumes) {
    ...
    function validateInput(req, res, next) {
        ...
        if (!result.status && result.errors && result.errors.length > 0) {
            let errorObj = new Error('Validation failed for ' + parameter.name);
            errorObj.status = 400;
            errorObj.details = result.errors;
            next(errorObj);
            return;
        }
        accessor.set(req, parameter.name, result.value);
        next();
    }
