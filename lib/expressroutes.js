 ...
 * @return {function}
 */
var buildNotAllowedMiddleware = function(methods) {
...
function expressroutes(router, options) {
    ...
    routes.forEach(function(route) {
        ...
        var path = buildRoutePath(mountpath, route.path);
        routesMethod[path] = routesMethod[path] || [];
        routesMethod[path].push(route.method.toLowerCase());
    });

    for(var path in routesMethod) {
        router.use(pathRegexp(path), buildNotAllowedMiddleware(routesMethod[path]));
    }
}
