...
function map(model) {
    if (thing.isString(model)) {
        return primitiveValidator(model);
    }
...
/**
 * Validate primitives outside tv4.
 * @param type
 * @returns {validate}
 */
function primitiveValidator(type) {
    var jtype = jsontype(type);
    ...
    return function validate(data, callback) {
        ...
        if (typeof value !== jtype) {
            error = new Error('invalid type: ' + (typeof value) + ' (expected ' + type + ')');
        }
