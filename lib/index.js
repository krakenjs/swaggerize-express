...
function swaggerize(options) {
    ...
    app.once('mount', assignSwagger(options));

    expressroutes(app, options);

    return app;
}

/**
 * Onmount handler.  Assigns swagger to the parent app once it is mounted.
 * @param options
 * @returns {onmount}
 */
function assignSwagger(options) {
