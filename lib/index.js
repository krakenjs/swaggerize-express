...
module.exports = function swaggycat(options) {
    ...
    assert.ok(validation.valid, 'Invalid specification: ' + (validation.error && JSON.stringify(validation.error)));

    if (thing.isString(options.routes) || !options.routes) {
        options.routes = options.routes && path.resolve(options.routes) || path.join(path.dirname(caller()), 'routes');
...
function mount(options) {
    ...
    //TODO:
    mountpath = options.api.resourcePath ? path.join(options.api.resourcePath, options.api.apiVersion || '') : '';
